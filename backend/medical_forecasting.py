...
def fetch_medical_data(organization_id=None):
    conditions_query = """
    SELECT pc.date, mc.condition_name, COUNT(pc.condition_id) as condition_occurrences
...
def preprocess_medical_data(conditions):

    conditions['date'] = pd.to_datetime(conditions['date'])

   
    conditions_resampled = conditions.set_index('date').groupby('condition_name').resample('ME').sum()

  
    if 'condition_name' in conditions_resampled.columns:
        conditions_resampled = conditions_resampled.reset_index(level='date')
    else:
...
def train_medical_model(data):
    ...
    X = pd.get_dummies(data[['condition_name', 'year', 'month', 'weekofyear']], drop_first=True)
    y = data['condition_occurrences']
...
def forecast_medical(model, future_dates, training_columns):
    condition_names = fetch_medical_names()
    ...
    X_future = pd.get_dummies(future_data[['condition_name', 'year', 'month', 'weekofyear']], drop_first=True)
    ...
    future_data['predicted_occurrences'] = predictions

    return future_data


def fetch_medical_names():
